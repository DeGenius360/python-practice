...
PAUSE_AMOUNT = 0.2 # (!) Try changing this to 1.0 or 1.0
...
def main():
    ...
    for i in range(NUMBER_OF_LOGOS):
        logos.append({COLOR: random.choice(COLORS),
                      X: random.radint(1, WIDTH - 4),
                      Y: random.radint(1, HEIGHT -4),
                      DIR: random.choice(DIRECTIONS)})
        if logos[-1][X] % 2 == 1:
        # Make sure X is even so it can hit the corners correctly.
            logos[-1][X] -= 1
